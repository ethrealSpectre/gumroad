...
describe FollowersController, inertia: true do
  ...
  context "within consumer area" do
    ...
    describe "POST create" do
      before do
        @request.env["HTTP_REFERER"] = seller.profile_url
      end

      it "creates a follower object and redirects back" do
      ...
      it "redirects with notice flash message for unconfirmed follower" do
        post :create, params: { email: "follower@example.com", seller_id: seller.external_id }
        expect(response).to redirect_to(seller.profile_url)
        expect(flash[:notice]).to eq("Check your inbox to confirm your follow request.")
      end

      it "redirects with alert flash message when email is invalid" do
      ...
      describe "logged in" do
        ...
        it "redirects with notice flash message for confirmed follower" do
